...
3. 启动应用时建表
    ...
    - FastAPI应用启动时，创建数据库表
"""
# 1. 定义异步函数：使用 async def 标记，表明该函数是可等待的（异步函数）
async def create_tables():
    # 2. 异步上下文管理器：获取数据库连接（conn），并自动管理连接的创建与释放
    #    async_engine.begin() 会创建一个带事务的异步连接，执行完成后自动提交事务（创建表无需手动回滚/提交）
    async with async_engine.begin() as conn:
        # 3. 异步执行同步方法：run_sync() 是 SQLAlchemy 异步连接的核心方法
        #    作用：将同步的 ORM 操作（Base.metadata.create_all）包装成可异步等待的操作
        #    Base.metadata.create_all：同步方法，扫描所有继承 Base 的 ORM 模型，自动在数据库中创建对应的表（不存在则创建，已存在则跳过）
        await conn.run_sync(Base.metadata.create_all)
# on_event为旧版本，新版为startup_event
...
async def get_database():
    async with AsyncSessionLocal() as session:
        try:
            yield session              # 返回数据库会话给路由处理函数(等待它处理完成后继续执行)
...
@app.get("/book/books")
async def get_book_list(db: AsyncSession=Depends(get_database)): # 注入依赖项
    # 查询
